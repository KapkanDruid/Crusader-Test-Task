...
{
    public static class CMSContainer
    {
        ...
        public static CMSEntity Get(string entityId)
        {
            ...
            if (entity == default)
            {
                LogUtil.LogError(nameof(CMSContainer), $"Unable to resolve entity id '{entityId}");
        ...
        private static void AutoCacheEntities()
        {
            ...
            foreach (var entityPrefab in entityPrefabs)
            {
                try
                {
                    ...
                    UnityEditor.EditorUtility.SetDirty(entityPrefab);
#endif
            ...
            LogUtil.Log(nameof(CMSContainer), stringBuilder + "\n", Color.green);
        }
    }
}
